...
estimates use us_college7
est sto us_college
estimates use us_skill7
est sto us_skill
...
esttab ch_college ch_skill using main_ch.tex, replace se coeflabel(sigeE_college "Skill" sigA_college "Age (2nd)" sigA2 "Age (1st)") mtitle("Education" "Occupation" "Education" "Occupation") drop() mgroups("Switzerland" "USA", pattern(1 0 1 0) span prefix(\multicolumn{@span}{c}{) suffix(}) erepeat(\cmidrule(lr){@span})) booktabs nonumber star(* 0.10 ** 0.05 *** 0.01) order(sigeE_college sigA2 sigA_college Trend) scalars(T) 


// CH only results, Industry
esttab ch_college_ind ch_skill_ind using ind_skill_ch.tex, replace se cells( (b(star fmt(3)) F(fmt(3)))se(par fmt(2)) ) drop(Trend* sigA*) nonumber coeflabel(sigE_coll_1 "Manufacturing" sigE_coll_2 "Finance" sigE_coll_3 "Construction" sigE_coll_4 "Trade \& Transport" sigE_coll_5 "IT" sigE_coll_6 "Other Services" sigE_coll_7 "Health")  star(* 0.10 ** 0.05 *** 0.01) scalars(T) mtitle("Education" "Occupation") booktabs 

esttab ch_college_ind ch_skill_ind using ind_age_ch.tex, replace se drop(Trend* sigE*) nonumber coeflabel(sigA_coll_1 "Manufacturing (2nd)" sigA_coll_2 "Finance (2nd)" sigA_coll_3 "Construction (2nd)" sigA_coll_4 "Trade \& Transport (2nd)" sigA_coll_5 "IT (2nd)" sigA_coll_6 "Other Services (2nd)" sigA_coll_7 "Health (2nd)" sigA2_coll_1 "Manufacturing (1st)" sigA2_coll_2 "Finance (1st)" sigA2_coll_3 "Construction (1st)" sigA2_coll_4 "Trade \& Transport (1st)" sigA2_coll_5 "IT (1st)" sigA2_coll_6 "Other Services (1st)" sigA2_coll_7 "Health (1st)")  star(* 0.10 ** 0.05 *** 0.01) scalars(T) mtitle("Education" "Occupation") booktabs order(sigA2_coll_1 sigA_coll_1 sigA2_coll_2 sigA_coll_2 sigA2_coll_3 sigA_coll_3 sigA2_coll_4 sigA_coll_4 sigA2_coll_5 sigA_coll_5 sigA2_coll_6 sigA_coll_6 sigA2_coll_7 sigA_coll_7) 
