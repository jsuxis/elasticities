...
program define cardlemieuxind, rclass
	...
	forval i=1/7 {
		...
		forval y=1980(2)2016 {
			forval a=2/8 {
				reg loglohn i.skilled i.geschl age i.nonwhite if jahr==`y' & altersgr==`a' & ind==`i', robust
	...
	save wage_reg_age_ind_college_us.dta, replace


	use labor_age_ind_college_us.dta, clear
	merge 1:1 jahr altersgr ind using wage_reg_age_ind_us.dta, nogen
	...
	save firststep_ind_us.dta, replace
	
* First Step
	gen outcome_u = .
	gen outcome_s = .
	
	append using firststep_us.dta
	...
	forval i=1/7 {
		...
		mat alpha_`i' = e(b)
		mat alpha_`i' = alpha_`i'[1,20..26]
		mat list alpha_`i'
		reg outcome_s i.jahr ibn.altersgr if ind == `i', nocon
		mat beta_`i' = e(b)
		mat beta_`i' = beta_`i'[1,20..26]
	...
	keep jahr ind L H
	save age_2step_ind_us.dta, replace
	
	use firststep_ind_us.dta, clear

	merge m:1 jahr ind using age_2step_ind_us.dta, nogen update
	append using firststep_us.dta
	merge m:1 ind jahr  using age_2step_us.dta, update
*	keep if altersgr >1 & altersgr <9
	gen ratio1 = log(H/L)
...
//	keep if jahr >= 1990
	...
	forval i=1/7 {
		ivreg2 logwage t i.altersgr (ratio1 ratio2 = loglabortotal loglabortotal2 )  [aw=wgt] if ind==`i', liml
